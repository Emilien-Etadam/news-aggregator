...
verify_post_install() {
  ...
  for js_file in reading-mode.js mark-as-read.js; do
    [ -f "assets/js/${js_file}" ] || err "Post-install check failed: assets/js/${js_file} missing (run bun build)"
  done
  log "Frontend bundles OK (reading-mode.js, mark-as-read.js)"

  [ -f assets/ts/reading-mode.ts ] || err "Post-install check failed: assets/ts/reading-mode.ts missing from repository"
  log "Reading mode TypeScript source OK"

  php bin/console debug:router app_article_show --no-ansi >/dev/null 2>&1 \
    || err "Post-install check failed: route app_article_show not registered"
  log "Reader page route OK (app_article_show)"
  ...
  php bin/console dbal:run-sql "SELECT column_name FROM information_schema.columns WHERE table_name = 'article' AND column_name = 'image_url'" --no-ansi 2>/dev/null \
    | grep -q image_url \
    || err "Post-install check failed: article.image_url column missing (run doctrine:migrations:migrate)"
  log "Database schema OK (article.image_url)"

  php bin/console list app:articles 2>/dev/null | grep -q backfill-images \
    || err "Post-install check failed: app:articles:backfill-images command missing"
  log "Backfill command OK (app:articles:backfill-images)"
}
